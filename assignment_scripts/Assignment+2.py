...
def answer_two():
    ...
    df['country'] = df.index
    return df.loc[df['diff'].idxmax(),'country']
answer_two()
...
def answer_three():
    ...
    only_gold = only_gold[only_gold['Gold.1'] > 0]
    only_gold['diff'] = (only_gold['Gold'] - only_gold['Gold.1'])/only_gold['Gold.2']
    only_gold['country'] = only_gold.index
    return only_gold.loc[only_gold['diff'].idxmax(),'country']
answer_three()
...
def answer_six():
    ...
    peeps = top_counties_df.groupby('STNAME').sum().sort_values(by='CENSUS2010POP' , ascending = False).head(3).index.tolist()
    return peeps
answer_six()
